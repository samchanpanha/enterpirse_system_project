...
-- Adds: branches table, branch_id column on every domain table, user_branches table
...
-- ─── 2. Seed HQ branch for existing demo tenant ──────────────────────────────
...
-- ─── 3. user_branches table (mapping users to branches with per-branch roles) ─
...
-- ─── 4. Helper: list of all tables in this DB (excludes flyway + branches) ───
DO $$
DECLARE
    rec RECORD;
...
BEGIN
    ...
    LOOP
        -- Add branch_id column (nullable initially; backfill; then NOT NULL)
        EXECUTE format('ALTER TABLE %I ADD COLUMN IF NOT EXISTS branch_id UUID', rec.table_name);
        -- Backfill: assign HQ branch to all existing rows
