...
public class JpaGatewayLogAdapter implements GatewayLogRepository {
    ...
    @Override public GatewayLog save(GatewayLog l) {
        ...
        e.setGateway(l.getGateway()); e.setRequestBody(l.getRequestBody()); e.setResponseBody(l.getResponseBody());
        e.setHttpStatus(l.getHttpStatus()); e.setDurationMs(l.getDurationMs()); e.setSuccess(l.getSuccess());
        e.setErrorMessage(l.getErrorMessage()); e.setCreatedAt(Instant.now());
        return toDomain(repo.save(e)); }
    private GatewayLog toDomain(GatewayLogEntity e) { return GatewayLog.builder().id(e.getId()).transactionId(e.getTransactionId()).gateway(e.getGateway()).requestBody(e.getRequestBody()).responseBody(e.getResponseBody()).httpStatus(e.getHttpStatus()).durationMs(e.getDurationMs()).success(e.getSuccess()).errorMessage(e.getErrorMessage()).createdAt(e.getCreatedAt()).build(); }
}