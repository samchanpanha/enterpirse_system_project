...
public class JpaWarehouseAdapter implements WarehouseRepository {
    ...
    @Override public Warehouse save(Warehouse w) {
        WarehouseEntity e = new WarehouseEntity(); e.setId(w.getId()); e.setTenantId(w.getTenantId());
    ...
    private Warehouse toDomain(WarehouseEntity e) { return Warehouse.builder().id(e.getId()).tenantId(e.getTenantId()).name(e.getName()).type(e.getType()).location(e.getLocation()).active(e.isActive()).createdAt(e.getCreatedAt()).build(); }
}